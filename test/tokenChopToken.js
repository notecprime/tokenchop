const TokenChopToken = artifacts.require("TokenChopToken");
...
contract("TokenChopToken", accounts => {
  it("should have a name", async () => {
    const factory = await TokenChopFactory.deployed();
    const instance = await TokenChopToken.at(await factory.allStable(0));
  ...
  it("can mint a stable token once transfer funds approved", async () => {
    ...
    const instance = await TokenChopToken.at(stableAddr);
